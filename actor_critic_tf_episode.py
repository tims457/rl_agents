...
class ActorCritic(tf.keras.Model):
    ...
    def learn(self, rewards, log_actions, v_preds, eps=1e-9):

        # create value targets
        v_targets = tf.TensorArray(dtype=tf.float32,
                                   size=len(rewards),
                                   dynamic_size=True)
        vt = rewards[-1]
        rewards = rewards[::-1]
        v_targets = v_targets.write(0, vt)
        for i in range(len(rewards) - 1):
            vt = rewards[i] + (self.gamma * vt)
            v_targets = v_targets.write(i + 1, vt)

        v_targets = v_targets.stack()[::-1]
        # normalize
        v_targets = (v_targets - tf.math.reduce_mean(v_targets)) / (
            tf.math.reduce_std(v_targets) + eps)

        # compute loss
        actor_loss = tf.math.reduce_sum(-log_actions * (v_targets - v_preds))
        critic_loss = tf.keras.losses.Huber()(v_targets, v_preds) * len(
            rewards)  #undo scaling
    ...
    def train(self, episodes, max_steps=1000):
        ...
        for episode in range(episodes):
            ...
            with tf.GradientTape() as self.tape:
                ...
                loss = self.learn(rewards, log_actions, v_preds)
